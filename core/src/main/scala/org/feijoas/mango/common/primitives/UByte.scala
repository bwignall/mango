...
final class UByte private (val value: Byte) extends AnyVal with Ordered[UByte] with Serializable {
   ...
   */
  def toString(radix: Int): String = UnsignedBytes.toString(value, radix)
...
object UByte {
   ...
   *  Byte}.
   */
  val MaxPowerOfTwo: Byte = 0x80.toByte

  /** The largest value that fits into an unsigned Byte.
   */
  val MaxValue: Byte = 0xff.toByte
  ...
  private[this] object UByteArrayOrdering extends Ordering[Array[UByte]] {
  ...
  implicit object UByteOrdering extends Ordering[UByte] {
    override def compare(x: UByte, y: UByte): Int = UnsignedBytes.compare(x.value, y.value)
  }
}
