...
private[mango] trait RangeMapBehaviors extends AnyFreeSpec with ScalaCheckPropertyChecks with MockitoSugar {
  this: AnyFreeSpec =>

  import RangeMapBehaviors._

  def aMutableRangeMapLike(newBuilder: => Builder[(TIntRange, String), TIntMutableRangeMapLike]): Unit = {
  ...
  def aRangeMapLike(newBuilder: => Builder[(TIntRange, String), TIntRangeMapLike]): Unit = {
